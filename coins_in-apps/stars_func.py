...
def perform_touch_and_update_coins(template_path, coin_increment, variable_to_match):
    ...
    screenshot_full_path = os.path.join(r'C:\\Users\\Hamza.Naseer\\Downloads\\Game In-Apps Automation using Airtest Project\\coins_in-apps\\Ehanced Images\\', screenshot_name)
    ...
    cropped_image = image.crop(subsequent_crop_box).convert('L')
    ...
    # Extract numbers from the enhanced cropped image
    ...
    if numbers:
        quantity = numbers[0] if len(numbers) > 0 else None  # Assuming the first number is the quantity
        price = numbers[1] if len(numbers) > 1 else None    # Assuming the second number is the price
        ...
        if quantity and price:
            ...
            print(f"Extracted Price: Rs {float(price):,.2f}")  # Formats the price with commas and two decimal points
        else:
